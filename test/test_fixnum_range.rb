...
class FixnumRangeTest < Minitest::Test
  def setup
  ...
  def test_range
    ...
    assert_equal(exp, Oj.dump(test, fixnum_range: (-1..1)))
  end
  ...
  def test_max_safe
    ...
    assert_equal(exp, Oj.dump(test, fixnum_range: :max_safe))
  end
  ...
  def test_valid_modes
    ...
    [:strict, :null, :compat, :rails, :custom].each do |mode|
      assert_equal(exp, Oj.dump(test, mode: mode, fixnum_range: (-1..1)), "Invalid mode #{mode}")
    end
